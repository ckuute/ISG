...
class App:
    def __init__(self, master):
        self.master = master
        self.master.title("VideoToFiles")
        ...
        self.label1 = tk.Label(self.frame1, text="VideoToFiles", font=("Calibri", 16))
        ...
        self.label2 = tk.Label(self.frame2, text="還原檔案資料夾", font=("Calibri", 16))
    ...
    def convert_files(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            state=executor.map(vtf.VTF,items)
        err=list(state).count("FileError")
        if err!=0:
            messagebox.showerror("執行結果","有 "+str(err)+" 個檔案錯誤！")
        else:
            messagebox.showinfo("執行結果","全部執行成功")
        self.file_list = []
        self.file_path = []
        self.update_treeview1()
        self.update_treeview2()
    ...
    def open_folder(self):
        ...
        if not os.path.exists(p):
            messagebox.showerror("錯誤: 資料夾不存在", "NOFOUND : C:/ISG/files")
            return
        else:
            os.startfile(p)
